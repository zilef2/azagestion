...
class RangoOrdenesCompra extends Component
{
    ...
    public function mount(){
        ...
        $this->nombreC = end($ListaControladoresYnombreClase);
        Log::info(' U:'.Auth::user()->name. ' Accedio a la vista ' .$this->nombreC );
    }
    ...
    public function DescargarOrdenes() {
        ...
        if($this->validate){
            try {
                return Excel::download(new ReportesExport($this->fechaini,$this->fechafin), 
                    'RelacionEntrega - ' .$this->arreglarFechas().'.xlsx');

                Log::info(' U:'.Auth::user()->name. ' descargo el archivo:  RelacionEntrega - ' .$this->arreglarFechas().'.xlsx en la vista ' .$nombreC );
            } catch (\Throwable $th) {
                ...
                Log::critical(' U:'.Auth::user()->name. ' intento descargar el excel en la vista ' .$nombreC .' ERROR: '.$th->getMessage());
            }
        }else{
            session()->flash('message', 'La fecha inicial tiene que ser inferior a la final.');
            Log::alert(' U:'.Auth::user()->name. ' intento descargar el excel en la vista ' .$nombreC .' Pero la fecha inicial debe ser anterior a la final' );
        }
    }
